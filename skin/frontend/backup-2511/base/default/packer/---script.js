...
function renderSidebarContent(){
	...
	jQuery.post('/diamonds/ajax/sidebarinfo/',{},function(response){
		...
		if( response.diamond_type ){
	        ...
	        jQuery('.setup-diamonds .item-price1').text( currCode + ' $' + formatDollar( parseFloat(response.diamond_data.diamonds_price) ) );
	        jQuery('.setup-diamonds .item-description').text( response.diamond_data.diamonds_name );
	        jQuery('.btn-view-diamond a').attr('href',  response.diamond_url + '?sidebar=true');
	        jQuery('img.sidebar-diamond').attr('src', 'media/wysiwyg/icotheme/diamonds_pics/' + response.diamond_data.shape.toLowerCase() + '_t.jpg' );
	        totalPrice += parseFloat( response.diamond_data.diamonds_price );
		}
		...
		if( response.ring != false ){
	        ...
	        jQuery('.setup-settings .item-price1').text( currCode + ' $' + formatDollar( parseFloat(response.ring_price_val) ) );
	        jQuery('.setup-settings .item-description').text( response.ring_name );
	        jQuery('.btn-view-ring a').attr('href', response.ring_url + '?sidebar=true' );
		...
		jQuery('.total-price').text( currCode + ' $' + formatDollar( parseFloat(totalPrice) )  );
