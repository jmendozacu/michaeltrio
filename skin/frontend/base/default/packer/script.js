...
function showSweetCart(params) {
	...
	var btnViewRing = params.btnViewRing ? btnViewRing = '<button href="javascript:;" class="button go-to-ring"><span><span>View Ring</span></span></button>' : '';
...
jQuery(document).ready(function (e) {
    ...
    jQuery(function ($) {
        ...
        //$(".appointment-form").on('submit', function () {
            $('.appointment-form .spinner-container').fadeIn();
            $('.appointment-form .alert').slideUp(100);

            var name = $(".appointment-form .name").val();
            var email = $(".appointment-form .email").val();
            var contact = $(".appointment-form .contact").val();
            var date = $(".appointment-form .date-picker").val();
            var time = $(".appointment-form .time-picker").val();
            var comments = $(".appointment-form .comments").val();
            var newsletter_signup = $('.appointment-form .c-newsletter-signup').prop('checked');
            var cProposalRing = $('.appointment-form .c-proposal-ring').prop('checked');
            var cWeddingRing = $('.appointment-form .c-wedding-ring').prop('checked');
            var cOther = $('.appointment-form .c-other').prop('checked');
            ...
            $.post(BASE_URL + 'trdforms/ajax/savecontactsform/',
                ...
                if (response.status == 'true' && response.message == 'success') {
                    ...
                    setTimeout($('.appointment-form .spinner').fadeOut(function () {
                        $('.appointment-form .success').fadeIn(function () {
                            var i = 5;
                            var refreshIntervalId = setInterval(function () {
                                if (i > 0) {
                                    i--;
        ...
        // ring_size_maker form submit
		
		// Attach a submit handler to the form
		$( ".ring_size_maker" ).submit(function( event ) {
		    
			$('.ring_size_maker .spinner-container').fadeIn();
            $('.ring_size_maker .alert').slideUp(100);

            var given_name = $(".ring_size_maker .given_name").val();
            var email = $(".ring_size_maker .email").val();
            var mobile = $(".ring_size_maker .mobile").val();
            var surname = $(".ring_size_maker .surname").val();
            var address = $(".ring_size_maker .address").val();
            var gender = $(".ring_size_maker .gender").val();
			var age = $(".ring_size_maker .age").val();
			var unit_no = $(".ring_size_maker .unit_no").val();
			var postal_code = $(".ring_size_maker .postal_code").val();
			
            var newsletter_signup = $('.ring_size_maker .c-newsletter-signup').prop('checked');
			console.log(given_name + ' ' + email + ' ' + mobile + ' ' + address);
		  ...
		  var posting = $.post(BASE_URL + 'trdforms/ajax/savefreeplasticringsizerform/',
                    {
		  ...
		  posting.done(function(data) {
			var response = JSON.parse(data);
			 ...
			 if (response.status == 'true' && response.message == 'success') {
                    ...
                    // $('.contact-form .btn-form-submit').slideUp();
                    setTimeout($('.ring_size_maker .spinner').fadeOut(function () {
                        $('.ring_size_maker .success').fadeIn(function () {
                            var i = 5;
                            var refreshIntervalId = setInterval(function () {
                                if (i > 0) {
                                    i--;
                                    ...
                                    clearInterval(refreshIntervalId);
                                    $('.closing-in').text('5');
                                    $('.ring_size_maker .spinner-container').hide()
                                    $('.ring_size_maker input').val('')
                                    $('.ring_size_maker textarea').val('')
                                    $('.ring_size_maker .c-proposal-ring').prop('checked', false);
                                    $('.ring_size_maker .c-wedding-ring').prop('checked', false);
                                    $('.ring_size_maker .c-other').prop('checked', false);
                                    $('.ring_size_maker .btn-form-submit').show();
                                    //grecaptcha.reset();
                                    iCheckActivate();
                                }
                            }, 1000);
					...
					/* Code by aslam */
					setTimeout(function(){ window.location.href = 'http://www.michaeltrio.com/successful-ring-sizer-request-form-submission'; }, 1000);
                } else if (response.message == 'captcha') {
                    $('.ring_size_maker .captcha-message').show();
                    setTimeout($('.ring_size_maker .spinner-container').fadeOut(), 1000);
                }
		  });
		  return false;
		});

       // $(".ring_size_maker").on('submit', function () {
//            $('.ring_size_maker .spinner-container').fadeIn();
//            $('.ring_size_maker .alert').slideUp(100);
//
//            var given_name = $(".ring_size_maker .given_name").val();
...
//            //responseField = $("input#recaptcha_response_field").val();
//
//            $.post(BASE_URL + 'trdforms/ajax/savefreeplasticringsizerform/',
//                    {
//                        given_name: given_name,
//                        email: email,
//                        mobile: mobile,
            ...
            if (email.val() === "" || !isRFC822ValidEmail(email.val())) {
                ...
                $.get("js/Mailchimp/subscribe-user.php?mc=1&email=" + email.val(), function (data) {
                    ...
                    if (data.error == "0" && data.result.email === email.val()) {
						...
						setTimeout(function () { window.location.href = 'http://www.michaeltrio.com/successful-newsletter-registration'; }, 500);
                        subsSetcookie();
                    } else {
                        email.focus();
                    }
  ...
  /*  jQuery('button.sidebar-button.sidebar-checkout').click(function (e) {
    ...
    jQuery('.select-diamond-step').click(function (e) {
        ...
        if (step == 1 || step == 2 || step == 3) {
            var goToStepUrl = BASE_URL + 'diamonds/diamond/gotostep/';
