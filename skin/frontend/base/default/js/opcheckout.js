...
//        //this.accordion.openNextSection(true);
                ...
                //tablefg=$('shopping-cart-totals-table');
                //tablefg.find('colgroup').remove();
                //var str = $('checkout-review-table').innerHTML;
                // $('shopping-cart-totals-table').update(str);
                // var dfvfd=response.update_section.html;
                //alert($("shopping-cart-totals-table > tr:nth-child(1) > td:nth-child(2)").html); 
                $$('#checkout-review-table tfoot').each(function(val, i) {
                    $('maintfoot').update(val.innerHTML);
                    });
                }
...
//            }
                ...
                //alert(response.customreview);
				if (response.customreview) {
                    $('maintfoot').update(response.customreview);
                }
...
var ShippingMethod = Class.create();
        ...
        this.saveUrl = saveUrl;
				...
				if (response.cashondelivery_chk=='0') {
					if($('p_method_cashondelivery') != undefined) {
					$$('label[for="p_method_cashondelivery"]').each(function(s){ 
						s.remove();
					});
					$('p_method_cashondelivery').remove();	
					}
                }
                }
...
var Review = Class.create();
        ...
        this.saveUrl = saveUrl;
                ...
                if (response.update_section) {
                //alert(response.update_section.html);
                $('checkout-' + response.update_section.name + '-load').update(response.update_section.html);
                }
