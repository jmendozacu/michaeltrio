...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action {
    ...
    public function deleteAction() {
        ...
        if ($id) {
            try {
                ...
                $skutext = $readConnection->fetchCol('SELECT product_id FROM sales_flat_quote_item WHERE `item_id` ="' . $id . '" ');
    ...
    public function stnajaxUpdateAction() {
        if (!$this->_validateFormKey()) {
            Mage::throwException('Invalid form key');
        }
        $id = (int) $this->getRequest()->getParam('id');
        ...
        if ($id) {
            try {
                ...
                $result['subtotal'] = $currSymbol.number_format($subtotal, 2, '.', '');;
...
public function stnajaxnewUpdateAction() {
        if (!$this->_validateFormKey()) {
            Mage::throwException('Invalid form key');
        }
        $id = (int) $this->getRequest()->getParam('id');
