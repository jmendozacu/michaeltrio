...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function successAction()
    {
	    ...
	    Mage::getSingleton('core/session')->unsCustomstndiscount();
            ...
            if (!isset($result['error'])) {
				...
				$customreview .='<tr>
                    ...
                    <td style="" class="a-right">
                        <span class="price">'.$currSymbol.number_format($subtotal, 2, '.', '').'</span> 
                    </td>
                </tr>';
				
				if (!empty($_deiscountArray)) {
					foreach($_deiscountArray as $_deiscount){
                    $val_deiscount=$_deiscount['name'];
					if(strpos(Mage::helper('checkout')->formatPrice(-$_deiscount['price']), '-'))
					{
                    ...
                    $customreview .='<tr>
                        ...
                        <td style="'.$newstyle.'" class="a-right">
							'.Mage::helper('checkout')->formatPrice(-$_deiscount['price']).'
                        </td>
                    </tr>';					
                 } }
				 ...
				 $customreview .='<tr>
                    ...
                    <td style="" class="a-right">
                        <span class="price">'.Mage::helper('core')->currency($newrates['flatrate_flatrate']).'</span> 
                    </td>
                </tr>';
				...
				$customreview .='<tr>
                    ...
                    <td style="" class="a-right">
                        <span class="price">'.$currSymbol.number_format($grandtotal, 2, '.', '').'</span> 
                    </td>
                </tr>';
				
				$result['customreview'] = $customreview;
            }
    ...
    public function saveShippingMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
			...
			if($data!='inchoo_shipping_large')
			{
			 $result['cashondelivery_chk'] = '0';
			}
			else
			{
			 $result['cashondelivery_chk'] = '1';
			}
    ...
    public function saveOrderAction()
    {
        ...
        try {
			...
			$new_deiscount_description=array();
			foreach($_deiscountArray as $_deiscountArrayval)
			{
			$new_deiscount_description[]=$_deiscountArrayval['name'];
			}
			$write_connection = Mage::getSingleton('core/resource')->getConnection('core_write'); 
			$fields_arr = array();
			$fields_arr['discount_description'] = implode(', ',$new_deiscount_description);		
			$where = $write_connection->quoteInto('increment_id =?', $Order_Id);
			$write_connection->update('sales_flat_order', $fields_arr, $where);
            ...
            foreach ($this->getOnepage()->getQuote()->getAllItems() as $item) {
                ...
                if ($productid != '') {
                   ...
                   // print_r($ids); 
                    if (in_array('103', $ids)) {
                        $customOptions = Mage::getModel('catalog/product_option')->getProductOptionCollection($_product);
                        foreach ($customOptions as $option) {
                            if($option->getTitle() == 'Product_A' || $option->getTitle() == 'Product_B'){
                                $values = Mage::getSingleton('catalog/product_option_value')->getValuesCollection($option);
                                //echo $option->getTitle().'<br><br>';
                                foreach ($values as $value) {
                                    //echo $value->getTitle().'<br><br>';
                                    //$productqty = Mage::getModel('catalog/product')->load($value->getTitle());
                                    $stockItem = Mage::getModel('cataloginventory/stock_item')->loadByProduct($value->getTitle());
