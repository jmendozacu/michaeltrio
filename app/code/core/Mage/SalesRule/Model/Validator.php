...
class Mage_SalesRule_Model_Validator extends Mage_Core_Model_Abstract
{
    ...
    public function process(Mage_Sales_Model_Quote_Item_Abstract $item)
    {
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            $_descriptionArrayANme = $rule->getStoreLabels();
            if($discountAmount != 0 && $discountAmount != ''){
                $data['name'] = $_descriptionArrayANme[0];
                $data['price'] = $discountAmount;
                $_deiscountArray[] = $data;
            }
        ...
        /*if(!empty($address->getDiscountDescriptionArraystn())){
            $datacustomstn = $address->getDiscountDescriptionArraystn();
        } else {
            $datacustomstn = array();
        }*/
        if(!empty($_deiscountArray) && !empty($address->getDiscountDescriptionArraystn())){
            $dtndata = $address->getDiscountDescriptionArraystn();
            $custonStnarr = array();
            foreach($_deiscountArray as $ky => $discou){
                $discou['price'] = $discou['price'] + $dtndata[$ky]['price'];
                $custonStnarr[] = $discou;
            }
            $_deiscountArray = $custonStnarr;
        }
/*        echo '<pre>';
        print_r(array_filter($_deiscountArray));
        echo '</pre>';*/
        //array_push($datacustomstn,$_deiscountArray);
        //$datacustomstn = array_sun($datacustomstn, $_deiscountArray);
        $address->setDiscountDescriptionArraystn($_deiscountArray);
        Mage::getSingleton('checkout/session')->setCustomstndiscount($_deiscountArray);
    ...
    public function prepareDescription($address, $separator = ', ')
    {
        ...
        $descriptionArray = $address->getDiscountDescriptionArraystn();
        ...
        $address->setDiscountDescription($descriptionArray);
        return $this;
    }
