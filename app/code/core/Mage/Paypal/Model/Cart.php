    ...
    protected function _render()
    {
        ...
        if ($this->_salesEntity instanceof Mage_Sales_Model_Order) {
            ...
            $this->_totals = array(
                ...
                //self::TOTAL_DISCOUNT => $this->_salesEntity->getBaseDiscountAmount(),
            ...
            $this->_totals = array (
                ...
                //self::TOTAL_DISCOUNT => $address->getBaseDiscountAmount(),
        ...
        if ($this->_isDiscountAsItem && $this->_totals[self::TOTAL_DISCOUNT]) {
            $discountcustom = Mage::getSingleton('checkout/session')->getCustomstndiscount();
            $textSisc = 'Discount';
            if(empty($discountcustom)){
                $textSisc = 'Discount';
            } else {
                foreach($discountcustom as $dc){
                    $textSisc .= $dc['name'].', ';                    
                }
                $textSisc = substr(trim($textSisc), 0, -1);
            }
            //print_r($discountcustom);
            $this->addItem(Mage::helper('paypal')->__($textSisc), 1, 1.00 * $this->_totals[self::TOTAL_DISCOUNT],
