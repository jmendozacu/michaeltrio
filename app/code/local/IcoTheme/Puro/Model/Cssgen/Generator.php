...
class IcoTheme_Puro_Model_Cssgen_Generator extends Mage_Core_Model_Abstract
{
    ...
    protected function _generateStoreCss($design, $storeCode)
    {
        ...
        $filedefault = Mage::helper('puro/cssgen')->getGeneratedCssDir() . $filename;
        $path = 'icotheme/puro/css/' . $design . '.phtml';
        ...
        try {
            ...
            if (empty($block)) {
                throw new Exception(Mage::helper('puro')->__("Template file is empty or doesn't exist: %s", $path));
            }
            $file = new Varien_Io_File();
            $file->setAllowCreateFolders(true);
            $file->open(array('path' => Mage::helper('puro/cssgen')->getGeneratedCssDir()));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('puro')->__('Failed generating CSS file: %s in %s', $filename, Mage::helper('puro/cssgen')->getGeneratedCssDir()) . '<br/>Message: ' . $gener->getMessage());
