...
set_include_path(get_include_path().PS.Mage::getBaseDir('app').DS.'code'.DS.'local'.DS.'IcoTheme'.DS.'Less'.DS.'Model');
require_once('lessc.inc.php');

class IcoTheme_Less_Model_Observer
{
    protected function _getHelper()
    {
        return Mage::helper('tless');
    }
    
    protected function _getConfigHelper()
    {
        return Mage::helper('tless/config');
    }
    ...
    protected function _getLessVariables($file)
    {
        ...
        Mage::dispatchEvent(
            'icotheme_less_additional_variables',
            array(
                'response'  => $response,
                'file_name' => $file,
            )
    ...
    protected function _getLessFunctions($file)
    {
        ...
        Mage::dispatchEvent(
            'icotheme_less_additional_functions',
            array(
                'response'  => $response,
                'file_name' => $file,
            )
    ...
    public function beforeLayoutRender($observer)
    {
        ...
        if (($head = $layout->getBlock('head'))
            ...
            // Cache by file path
            $filesCollection = Mage::getModel('tless/file')
                ->getCollection()
                ->load();
                    ...
                    if (substr($item['name'], -5) == '.less') {
                        ...
                        try {
                            ...
                            if (!is_array($oldCache) || ($newCache['updated'] > $oldCache['updated'])) {
                                ...
                                if ($isNewModel) {
                                    $model = Mage::getModel('tless/file')->setPath($baseFile);
                                }
