 ...
 */
class IcoTheme_ColorSwatches_Helper_Mediafallback extends Mage_Core_Helper_Abstract
{
    ...
    {
        $listSwatchAttr = Mage::helper('colorswatches/productlist')->getSwatchAttribute();
        ...
        $configAttributes = Mage::getResourceModel('colorswatches/catalog_product_attribute_super_collection')
            ...
            foreach ($configAttributes as $attribute) {
                ...
                foreach ($parentProduct->getChildrenProducts() as $childProduct) {
                    ...
                    $optionLabels = array_map(function ($value) {
                        return array_map('IcoTheme_ColorSwatches_Helper_Data::normalizeKey', $value);
                    }, $optionLabels);
    ...
    protected function _resizeProductImage($product, $type, $keepFrame, $image = null, $placeholder = false)
    {
        ...
        if ($hasTypeData || $placeholder || $image) {
            ...
            if ($type == 'small_image') {
                $helper = Mage::helper('catalog/image')
                    ->init($product, $type, $image);
                $w = Mage::getStoreConfig(IcoTheme_ColorSwatches_Helper_Data::CONFIG_PRODUCT_LIST_IMG_WIDTH);
                $h = Mage::getStoreConfig(IcoTheme_ColorSwatches_Helper_Data::CONFIG_PRODUCT_LIST_IMG_HEIGHT);
                if (is_numeric($w)) {
                    $w = Mage::getStoreConfig(IcoTheme_ColorSwatches_Helper_Data::CONFIG_PRODUCT_LIST_IMG_WIDTH);
                    $h = Mage::getStoreConfig(IcoTheme_ColorSwatches_Helper_Data::CONFIG_PRODUCT_LIST_IMG_HEIGHT);
    ...
    protected function _getChildrenProductsAttributes()
    {
        return array(
            ...
            'image_label',
            'small_image_label',
            Mage::helper('colorswatches/productlist')->getSwatchAttribute()->getAttributeCode(),
        );
    }
    ...
    public function attachChildrenProducts(array $products, $storeId)
    {
        ...
        $collection = Mage::getResourceModel(
            'colorswatches/catalog_product_type_configurable_product_collection');
