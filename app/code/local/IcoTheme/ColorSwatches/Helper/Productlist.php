...
/**
 * Class IcoTheme_ColorSwatches_Helper_Productlist
 */
class IcoTheme_ColorSwatches_Helper_Productlist extends Mage_Core_Helper_Abstract
{
    /**
        ...
        if (Mage::helper('colorswatches')->isEnabled()
            ...
            if ($layer = $block->getLayer()) {
                foreach ($layer->getFilterableAttributes() as $attribute) {
                    if (Mage::helper('colorswatches')->attrIsSwatchType($attribute)) {
                        $block->getChild($attribute->getAttributeCode() . '_filter')
                            ->setTemplate('icotheme/colorswatches/catalog/layer/filter/swatches.phtml');
                    }
                }
            }
            ...
            if ($stateRenderersBlock = $block->getChild('state_renderers')) {
                $swatchRenderer = Mage::app()->getLayout()
                    ->addBlock('colorswatches/catalog_layer_state_swatch', 'product_list.swatches');
                $swatchRenderer->setTemplate('icotheme/colorswatches/catalog/layer/state/swatch.phtml');
    ...
    public function getSwatchAttributeId()
    {
        if (is_null($this->_swatchAttributeId)) {
            $this->_swatchAttributeId =
                Mage::getStoreConfig(IcoTheme_ColorSwatches_Helper_Data::CONFIG_PATH_LIST_SWATCH_ATTRIBUTE);
        }
