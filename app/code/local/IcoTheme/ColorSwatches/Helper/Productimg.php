...
/**
 * Class IcoTheme_ColorSwatches_Helper_Productimg
 */
class IcoTheme_ColorSwatches_Helper_Productimg extends Mage_Core_Helper_Abstract
{
    /**
    ...
    const MEDIA_IMAGE_TYPE_MEDIA = 'media_image';
    ...
    public function getProductImgByLabel($text, $product, $type = null)
    {
        ...
        $text = IcoTheme_ColorSwatches_Helper_Data::normalizeKey($text);
     ...
     * @return IcoTheme_ColorSwatches_Helper_Data
     */
    public function indexProductImages($product, $preValues = null)
    {
        ...
        if (!isset($this->_productImagesByLabel[$product->getId()])) {
            ...
            if (!is_null($preValues) && is_array($preValues)) { // If a pre-defined list of valid values was passed
                $preValues = array_map('IcoTheme_ColorSwatches_Helper_Data::normalizeKey', $preValues);
                ...
                foreach ($attributes as $attribute) {
                    if (Mage::helper('colorswatches')->attrIsSwatchType($attribute->getAttributeId())) {
                        foreach ($attribute->getPrices() as $option) { // getPrices returns info on individual options
                            $searchValues[] = IcoTheme_ColorSwatches_Helper_Data::normalizeKey($option['label']);
                        }
                    }
                }
            }
            ...
            $imageHaystack = array_map(function ($value) {
                return IcoTheme_ColorSwatches_Helper_Data::normalizeKey($value['label']);
            }, $mediaGallery['images']);
    ...
    )
    {
        ...
        $value = IcoTheme_ColorSwatches_Helper_Data::normalizeKey($value);
        ...
        if ($object instanceof Mage_Catalog_Model_Layer_Filter_Item) { // fallback for swatches loaded for nav filters
            ...
            foreach ($source->getAllOptions(false, true) as $option) {
                if ($option['value'] == $object->getValue()) {
                    $defaultValue = IcoTheme_ColorSwatches_Helper_Data::normalizeKey($option['label']);
                    break;
                }
            }
        ...
        do {
            $filename = Mage::helper('colorswatches')->getHyphenatedString($value) . $fileExt;
    ...
    public function filterImageInGallery($product, $image)
    {
        ...
        if (!isset($this->_productImageFilters[$product->getId()])) {
            ...
            $filters = array_merge($filters, array_map(function ($label) {
                return $label . IcoTheme_ColorSwatches_Helper_Productimg::SWATCH_LABEL_SUFFIX;
            }, $filters));
            $this->_productImageFilters[$product->getId()] = $filters;
        }

        return !in_array(IcoTheme_ColorSwatches_Helper_Data::normalizeKey($image->getLabel()),
