 ...
 */
class IcoTheme_ColorSwatches_Model_Observer extends Mage_Core_Model_Abstract
{
    /**
    ...
    {
        if (!Mage::helper('colorswatches')->isEnabled()) { // check if functionality disabled
        ...
        if ($collection
            instanceof IcoTheme_ColorSwatches_Model_Resource_Catalog_Product_Type_Configurable_Product_Collection
        ) {
            // avoid recursion
            return;
        }
        ...
        foreach ($products as $product) {
            ...
            Mage::helper('colorswatches/productimg')
    ...
    public function convertLayerBlock(Varien_Event_Observer $observer)
    {
        ...
        if (($front == 'catalog' && $controller == 'category' && $action == 'view')
            ...
            Mage::helper('colorswatches/productlist')->convertLayerBlock($blockName);
        }
    }
}
