...
//                $product_id = 2624; //use your own product id
...
//                exit;
        ...
        $qtycount = Mage::helper('checkout/cart')->getSummaryCount();  //get total items in cart
        // $result = ['status' => $this->status, 'message' => $this->message];
        $result = array('status' => $this->status, 'message' => $this->message, 'qty' => $qtycount);
...
/*            echo $item->getId();
        ...
        if (count($_diamondItems)) {
            ...
            foreach ($_diamondItems as $item) {
                    ...
                    if (!empty($mainarray)) {
                        ...
                        foreach ($mainarray as $mainarraydata) {
                            ...
                            if ($mainarraydata['title'] == 'Product_A') {
								...
								$itemconvertedPrice = Mage::helper('core')->currency($itemorigPrice);
				...
				$updateurl= "'".Mage::getBaseUrl().'checkout/cart/ajaxUpdate/id/'.$item->getId()."'";
				
				$forlistcart .='<p class="row-fluid show-grid">
							Quantity:             
							<a href="javascript:void(0);" onclick="updatediamonddecreaseQty('.$product->getId().','.$updateurl.');" class="decrement_qty">
								<span class="fa fa-minus"></span>
							</a>
							<input id="qinput-'.$product->getId().'" data-link="'.Mage::getBaseUrl().'checkout/cart/ajaxUpdate/id/'.$item->getId().'/uenc" data-item-id="'.$_item->getId().'" class="qty cart-item-quantity input-text input_q" name="" value="'.$item->getQty().'" readonly />
							<a href="javascript:void(0);" onclick="updatediamondincreaseQty('.$product->getId().','.$updateurl.');" class="increment_qty">
				...
				$forlistcart .='<p class="row-fluid show-grid">
							...
							<input id="qinput-'.$product->getId().'" data-link="'.Mage::getBaseUrl().'checkout/cart/ajaxUpdate/id/'.$item->getId().'/uenc" data-item-id="'.$product->getId().'" class="qty cart-item-quantity input-text input_q" name="" value="'.$item->getQty().'" readonly />
							<a href="javascript:void(0);" onclick="updatediamondincreaseQty('.$product->getId().','.$updateurl.');" class="increment_qty">
								<span class="fa fa-plus"></span>
							</a>
						</p>';
				...
				$price = Mage::helper('core')->currency($item->getCustomPrice());
        ...
        if (count($_otherProducts)) {
            ...
            foreach ($_otherProducts as $item) {
                ...
                $mainitemconvertedPrice = Mage::helper('core')->currency($mainitemorigPrice);
...
//echo   $customProductSkuId;die;
            ...
            $ringAttrbute.='Engagement Ring Name : ' . $ringproduct->getName() . "<br /> <hr>";
			$ringAttrbute.='Engagement Ring Sku : ' . $ringproduct->getSku() . "<br />";
			...
			$diamondAttrbute.='Diamond Name : ' . $Diamondproduct->getName() . "<br />";
			$diamondAttrbute.='Diamond Sku : ' . $Diamondproduct->getSku() . "<br />";
            ...
            $product->setDescription(str_replace("<br />","\n",$ringAttrbute . $diamondAttrbute));
            $product->setShortDescription(str_replace("<br />","\n",'Custom ring made by diamond and setting. <br /> Ring Engraving <hr>' . $ringengravingAttrbute));
    ...
    public function gotostepAction() {
            ...
            switch ($data['step']) {
                case '2':
                    if ($data['diamond_id'] && $data['url']) {
                        ...
                        $data['url'] = Mage::getBaseUrl() . 'diamond.php';
                        Mage::getSingleton('core/session')->setDiamondStep($data['step']);
                        Mage::getSingleton('core/session')->setStepOneUrlState($data['url']);
                        $this->message = 'success';
                        $this->status = 'true';
                    }
                    $url = Mage::getBaseUrl() . 'engagement-rings.html?step=2';
                    break;
                case '1';
                    Mage::getSingleton('core/session')->unsSettingStyle();
                    Mage::getSingleton('core/session')->setDiamondStep($data['step']);
					...
					if ($data['cut']) {
                        Mage::getSingleton('core/session')->setSDiamondCut($data['cut']);
                    } else {
                        Mage::getSingleton('core/session')->unsSDiamondCut();
                    }
                    $url = Mage::getSingleton('core/session')->getStepOneUrlState();
                    if (!$url) {
                        $url = Mage::getBaseUrl() . 'diamond.php';
                    }
                    break;
                case 'wedding';

                    Mage::getSingleton('core/session')->setWeddingStep($data['step']);
                    $this->message = 'success';
                    $this->status = 'true';
                    if ($data['maincat']) {
                        Mage::getSingleton('core/session')->setWeddingMaincat($data['maincat']);
                    } else {
                        Mage::getSingleton('core/session')->setWeddingMaincat();
                    }
                    if ($data['weddingstyle']) {
                        Mage::getSingleton('core/session')->setWeddingStyle($data['weddingstyle']);
                    } else {
                        Mage::getSingleton('core/session')->unsWeddingStyle();
                    }
                    $url = Mage::getBaseUrl() . 'wedding.html?weddingstep=wedding';
                    break;
			case 'jewellerygift';
                    Mage::getSingleton('core/session')->setWeddingStep($data['step']);
                    $this->message = 'success';
                    ...
                    if ($data['maincat']) {
					   ...
					   switch ($data['maincat']) {
							case 'necklace':
								$mainjewelleryurl="jewellery-necklaces.php";					
								break;
							case 'bracelet':
								$mainjewelleryurl="jewellery-bracelets.php";	
								break;
							default:
                                $mainjewelleryurl="jewellery-gifts.php"; 
						}
						
                        Mage::getSingleton('core/session')->setJewellerygiftMaincat($data['maincat']);
                    } else {
                        Mage::getSingleton('core/session')->setJewellerygiftMaincat();
                    }
                    if ($data['style']) {
                        Mage::getSingleton('core/session')->setJewellerygiftStyle($data['style']);
                    } else {
                        Mage::getSingleton('core/session')->unsJewellerygiftStyle();
                    }
                    $url = Mage::getBaseUrl() . 'jewellery-gifts.html?jewellerygiftstep=jewellerygift';
                    break;
			case 'jewellerygiftsale';
                    Mage::getSingleton('core/session')->setWeddingStep($data['step']);
                    $this->message = 'success';
                    $this->status = 'true';
                    if ($data['maincat']) {
                    ...
                    if ($data['styletype']) {
                        Mage::getSingleton('core/session')->setJewellerygiftStyletype($data['styletype']);
                    } else {
                        Mage::getSingleton('core/session')->unsJewellerygiftStyletype();
                    }
                    $url = Mage::getBaseUrl() . 'jewellery-gifts.html?styletype=jewellerygiftsale';
                break;	
			case 'fancydiamondstyle';
                    Mage::getSingleton('core/session')->setFancydiamondStep($data['step']);
                    $this->message = 'success';
                    $this->status = 'true';

                    if ($data['fancystyle']) {
                        Mage::getSingleton('core/session')->setFancydiamondStyletype($data['fancystyle']);
                    } else {
                        Mage::getSingleton('core/session')->unsFancydiamondStyletype();
                    }
                    $url = Mage::getBaseUrl() . 'fancy-color-diamond.html';
                break;	
            }
        ...
        //$result = ['status' => $this->status, 'message' => $this->message];
        $result = array('status' => $this->status, 'message' => $this->message, 'mainjewelleryurl' => $mainjewelleryurl);
        if ($url) {
            $result['url'] = $url;
        }
