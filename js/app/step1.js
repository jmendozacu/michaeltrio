...
jQuery(document).ready(function (e) {
    ...
    jQuery(function ($) {
        ...
        if ($('.page-diamonds').hasClass('cat-results')) {
            ...
            $(document).on('click', '.info_title', function (e) {
                e.preventDefault();
                $('.mypop').hide();
            ...
            jQuery(document).on('click', '.filter_slider', function (e) {
                e.preventDefault();
            ...
            jQuery(document).on('click', '.closeItemInfo', function (e) {
                e.preventDefault();
			...
			$(document).on('click', '.adv-filters', function (e) {
                e.preventDefault();
            ...
            function slidersInit(inputArray) {
                jQuery(".rangeslider").each(function (index, element) {
                    ...
                    var _Min = parseFloat($.trim(jQuery(this).attr('data-min')));
                    var _Max = parseFloat($.trim(jQuery(this).attr('data-max')));
                    ...
                    function rangeSlide(event, ui) {
                        ...
                        jQuery(event.target).closest('.m-filter-item-list').find('.rangemin').val(ui.values[ 0 ] + _suffix);
                        ...
                        jQuery(event.target).closest('.m-filter-item-list').find('.rangemax').val(ui.values[ 1 ] + _suffix);
            ...
            $(document).ready(function () {
				...
				$( ".prices .rangemin" ).blur(function() {
						diamondsParamsURL.price_to = $(".prices .rangemax").val();							   
				        diamondsParamsURL.price_from = $(this).val();
                 		jQuery(".prices .rangeslider").slider("values", 0, parseFloat(jQuery(this).val())); 
                        getDiamondsContent(diamondsParamsURL, true);
				});
				
				$( ".prices .rangemax" ).blur(function() {
                        diamondsParamsURL.price_to = $(this).val();
						diamondsParamsURL.price_from = $(".prices .rangemin").val();
                 		jQuery(".prices .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
				});
				
				$('.carat .rangemin').blur(function () {
						diamondsParamsURL.carat_to = $(".carat .rangemax").val();	
                        diamondsParamsURL.carat_from = $(this).val();
		                jQuery(".carat .rangeslider").slider("values", 0, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                });
                $('.carat .rangemax').blur(function () {
                        diamondsParamsURL.carat_to = $(this).val();	
						diamondsParamsURL.carat_from = $(".carat .rangemin").val();	
		                jQuery(".carat .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                });
				$('.depth .rangemin').blur(function () {
                        diamondsParamsURL.depth_to = $(".depth .rangemax").val();
                        diamondsParamsURL.depth_from = $(this).val();
                 		jQuery(".depth .rangeslider").slider("values", 0, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                    
                });
                $('.depth .rangemax').blur(function () {
                        diamondsParamsURL.depth_to = $(this).val();
						diamondsParamsURL.depth_from = $(".depth .rangemin").val();
 		                jQuery(".depth .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                    
                });
				$('.table-range .rangemin').blur(function () {
                        diamondsParamsURL.table_to = $('.table-range .rangemax').val();
                        diamondsParamsURL.table_from = $(this).val();
		                jQuery(".table-range .rangeslider").slider("values", 0, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                    
                });
                $('.table-range .rangemax').blur(function () {
                        diamondsParamsURL.table_to = $(this).val();
						diamondsParamsURL.table_from = $('.table-range .rangemin').val();
		                jQuery(".table-range .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                    
                });
				
                $('.prices .rangemin').keydown(function (e) {
                ...
                $('.prices .rangemax').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.price_to = $(this).val();
						diamondsParamsURL.price_from = $(".prices .rangemin").val();
                 		jQuery(".prices .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                ...
                $('.carat .rangemin').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.carat_from = $(this).val();
						diamondsParamsURL.carat_to = $(".carat .rangemax").val();
		                jQuery(".carat .rangeslider").slider("values", 0, parseFloat(jQuery(this).val()));
                ...
                $('.carat .rangemax').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.carat_to = $(this).val();	
						diamondsParamsURL.carat_from = $(".carat .rangemin").val();
		                jQuery(".carat .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                ...
                $('.depth .rangemin').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.depth_from = $(this).val();
                 		jQuery(".depth .rangeslider").slider("values", 0, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                ...
                $('.depth .rangemax').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.depth_to = $(this).val();
 		                jQuery(".depth .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                ...
                $('.table-range .rangemin').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.table_from = $(this).val();
		                jQuery(".table-range .rangeslider").slider("values", 0, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
                ...
                $('.table-range .rangemax').keydown(function (e) {
                    if (e.keyCode == 13 || e.keyCode == 9) {
                        diamondsParamsURL.table_to = $(this).val();
		                jQuery(".table-range .rangeslider").slider("values", 1, parseFloat(jQuery(this).val()));
                        getDiamondsContent(diamondsParamsURL, true);
            ...
            function setUIconfig(inputArray) {
                ...
                function setRangeSliderCut(instanceArray, currentArray, selector) {
                    if (typeof currentArray != 'undefined') {
                        ...
                        if(diamond_cut_sing)
						{
						jQuery(selector + ' .rangeslider').slider('values', 0, 3);
                        jQuery(selector + ' .rangeslider').slider('values', 3, 4);
						}
						else
						{
                        jQuery(selector + ' .rangeslider').slider('values', 0, valueFrom);
            ...
            function getDiamondsContent(params, isFiltering) {
                ...
                jQuery.post(BASE_URL + 'diamonds/ajax/filter/',
                        ...
                        function (data) {
							...
							{
							   jQuery('#newdaimonds').remove();
							}
							else
							{
					   jQuery('#newdaimonds').remove();
					   jQuery("table.diamonds-table-content").before(jQuery('<p id="newdaimonds" class="note-msg newdaimonds">There are no products matching the selection.</p>'));
							}
            ...
            function renderFilteredElements(inputArray, isFiltering) {
                ...
                inputArray.each(function (entry) {
                    //console.log(entry.diamond_table);
                    var cut = '';
                    ...
                    var elementToAppend = '<tr data-id="' + entry.entity_id + '" class="item_ajax_get_details ' + evenval_class + '">' +
                            ...
                            '<td class="cell-_table">' + entry.diamond_table + '</td>' +
                            '<td class="cell-cut">' + cut + '</td>' +
                            '<td class="cell-polish">' + polish + '</td>' +
                            '<td class="cell-symm">' + symmetry + '</td> ' +
                            '<td class="cell-price">' + entry.price + ' <i class="fa fa-arrow-right pl30"></i></td>' +
                            '</tr>';
