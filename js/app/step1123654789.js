jQuery(document).ready(function (e) {
    ...
    jQuery(function ($) {
        ...
        if ($('.page-diamonds').hasClass('cat-results')) {
            ...
            jQuery(document).on('click', '.removeCompare', function (e) {
                e.preventDefault();
                e.stopPropagation();
                var _tmptr = jQuery(this).closest('tr');
                var _tmpid = _tmptr.attr('data-id');
                jQuery('.item_ajax_get_details[data-id="' + _tmpid + '"]').find('.compare_item').trigger('click');
                _tmptr.slideUp(400, function () {
                    _tmptr.remove();
                })
            });
            ...
            function setUIconfig(inputArray) {
                ...
                jQuery('.prices input.rangemin').val(_prefix + ' ' + inputArray.price_from);
                jQuery('.prices .rangeslider').slider('values', 0, inputArray.price_from);
                jQuery('.prices input.rangemax').val(_prefix + ' ' + inputArray.price_to);
            ...
            if ((typeof price_from !== 'undefined') &&
                ...
                if (shapesArray.length === 0) {
                    shapesArray = ['RD', 'RA', 'PS', 'PR', 'OV', 'MQ', 'HS', 'EC', 'CU', 'AS'];
                }
